 ...
 *
 * @return: Ponteiro da tabela hash criada
 *
 */
PlayersHashTable *players_hash_table_init(uint size) {
    PlayersHashTable *players_hash_table = (PlayersHashTable *)malloc(sizeof(uint) + size * sizeof(PlayersList));
    players_hash_table->size = size;

    // Inicizaliza todas listas da tabela
    for (uint i = 0; i < size; ++i) players_list_init(&players_hash_table->players_hash_table[i]);
 ...
 *
 * @players_hash_table: Endereço de uma tabela hash
 */
...
PlayerData *players_hash_table_insertion(PlayersHashTable *players_hash_table, PlayerData data) {
    ...
    return players_list_insertion_begin(&(players_hash_table->players_hash_table[hash_func(data.fifa_id, players_hash_table->size)]), data);
}
 ...
 *
 * @players_hash_table: Endereço de uma tabela
...
PlayerData *players_hash_table_search(PlayersHashTable *players_hash_table, int key) {
    ...
    return players_list_search(players_hash_table->players_hash_table[hash_func(key, players_hash_table->size)], key);
}
