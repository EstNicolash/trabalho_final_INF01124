...
void insert_trie(TRIE tree, char player_name[], int player_id)
{
    ...
    while(offset < length)
    {
        ...
        if(hasChild(current_node, (int) current_char)) //se o nodo já possui a próxima letra, pula para ela (índice da letra no vetor é seu valor ascii)
        {
...
trienode *findEndOfPrefix(trienode *root, char *prefix) //posiciona o ponteiro no nodo correspondente a última letra do prefixo
{
    int offset = 0;
...
listnode* list_all(TRIE tree, char prefix[]) //retorna lista com todos os jogadores encotrados
{
    trienode *root;
...
void list_nodes(listnode **list_of_players, char *current_word, trienode *current_node) //a partir da útlima letra do prefixo, lista todos as palavras encontradas
{
    char *next_word = NULL;
