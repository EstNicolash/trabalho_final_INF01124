...
trienode *initialize_node(char value) {
    ...
    new_node->isEnd = 0;
    for (i = 0; i < ALPHABET_SIZE; i++) { new_node->children[i] = NULL; }
...
void insert_trie(TRIE tree, char player_name[], int player_id) {
    ...
    while (offset < length) {
        ...
        if (hasChild(current_node,
                     (int)current_char))  // se o nodo j� possui a pr�xima letra,
                                          // pula para ela (�ndice da letra no vetor
                                          // � seu valor ascii)
        {
            aux = current_node->children[(int)current_char];
            current_node = aux;
        } else  // sen�o, aloca nova letra/nodo
        {
...
void list_nodes(listnode **list_of_players, char *current_word,
                trienode *current_node)  // a partir da �tlima letra do prefixo,
                                         // lista todos as palavras encontradas
{
    ...
    if (next_word != NULL) { free(next_word); }
}
